...
Następnie
przedstawiono dwa znane warianty ewolucji różnicowej (DE/rand, DE/best) oraz 
...
Na funkcjach separowalnych, dobrze uwarunkowanych numerycznie lub unimodalnych ewolucja różnicowa znajdowała optimum na tyle często, 
że kłopotliwe byłoby porównywanie jej wariantów. 
Bardzo prawdopodobne byłoby to, że każdy z testowanych wariantów algorytmu znalazłby optimum. 
Różnice pomiędzy wariantami algorytmów łatwiej było uchwycić przy użyciu trudniejszych funkcji, na których optimum było rzadko znajdowane.
Wówczas liczyła się odległość od optimum, która wyróżniała niektóre algorytmy nad pozostałymi. 
...
Ekstrema w oryginalnej funkcji Rastrigina są rozmieszczone regularnie i symetryczne. W~BBOB2013 dzięki transformacjom
...
zasięgu. 
Wartość bliską 21,4 można uzasadnić dzięki wyprowadzonej zależności \ref{eq:macierz_kow_mutanta},
...
$\C[u_i] = \C[P](2 \cdot 0,9^2 + 1) = C[P] \cdot 2,62 \approx I_{10} \cdot 21,8$, gdzie $I_{10}$
to 10-wymiarowa macierz jednostkowa. 
...
ponieważ leżały poza obszarem przeszukiwań. Dla jednego algorytmu na jednej funkcji, procent osobników
...
Wraz ze wzrostem liczby wymiarów, początkowo średni odsetek osobników poza obszarem przeszukiwań również 
wzrasta, będąc najwyższy dla 20 lub 40 wymiarów, w zależności od funkcji. Jednak wraz z dalszym
wzrostem liczby wymiarów, procent osobników poza obszarem przeszukiwań spada. 
...
ma symetryczny rozkład prawdopodobieństwa. W przypadku DE/mid oraz DE/best, przesuwany jest
jeden ustalony punkt (środkowy albo najlepszy), a więc rozkład mutanta dalej jest symetryczny.
Natomiast w przypadku DE/rand przesuwany jest punkt losowy, co zaburza symetrię rozkładu mutanta.
Na podstawie przeprowadzonych eksperymentów nie można określić czy symetria rozkładu mutantów ma 
istotny wpływ na jakość otrzymywanych wyników. DE/mid (symetryczy) zajął pierwsze miejsce, 
...
Cel pracy został spełniony -- poprawiono efektywność algorytmu ewolucji różnicowej proponując
nowy wariant algorytmu nazwany DE/mid. Ponadto, szczegółowo przeanalizowano, zaimplementowano 
...
Eksperymenty wykazały znaczną przewagę
DE/mid na 6 z 7 wielowymiarowych funkcji testowych.
